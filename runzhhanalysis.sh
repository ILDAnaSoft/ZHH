...
Marlin /afs/desy.de/group/flc/pool/$(whoami)/zhh/ZHH/scripts/zhhanalysis.xml --constant.ILDConfigDir="$ILD_CONFIG_DIR" --constant.ZHH_REPO_ROOT="$REPO_ROOT" --constant.OutputDirectory="${outdir}/${10}" --constant.OutputBaseName="${1}" --global.MaxRecordNumber="${2}" --global.SkipNEvents="${3}" --global.LCIOInputFiles="${4}" --constant.CMSEnergy="${5}" --constant.ISRPzMax="${6}" --constant.NumberOfHiggs="${7}" --constant.NumberOfJets="${8}" --constant.RunKinfit="${9}" --constant.WhichSignature="${10}" --constant.errorflowconfusion="${11}" --constant.propagateerrorflowconfusion="${12}" --constant.useneutralpfocorrection="${13}"
