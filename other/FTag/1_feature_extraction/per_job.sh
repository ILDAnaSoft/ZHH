...
if [[ $outputs_exist_all = "False" ]]; then
    ...
    echo "Running Marlin"
    Marlin "$REPO_ROOT/scripts/dev_flavortag.xml" --global.LCIOInputFiles=$raw_file --constant.ILDConfigDir=$ILD_CONFIG_DIR  --global.MaxRecordNumber=0 --constant.OutputSuffix=$suffix
