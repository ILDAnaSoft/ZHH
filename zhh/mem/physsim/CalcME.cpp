...
// MEElement
MEElementPhyssimZHH::MEElementPhyssimZHH(PhysConstants *consts, double pol_e, double pol_p, int z_decay_mode) {
    setConstants(consts);
...
MEElementPhyssimZZH::MEElementPhyssimZZH(PhysConstants *consts, double pol_e, double pol_p, int z1_decay_mode, int z2_decay_mode) {
    setConstants(consts);
...
double MEElementPhyssimZHH::single(std::vector<double*> momenta) {
    /* 
    momenta[0-1]: e- e+
    momenta[2-3]: mu- mu+
    momenta[4-5]: b bbar
...
double MEElementPhyssimZZH::single(std::vector<double*> momenta) {
    /* 
    momenta[0-1]: e- e+
    momenta[2-3]: mu- mu+
    momenta[4-5]: b bbar
...
calc_me::calc_me(double energy) // , std::string param_card)
{
...
void calc_me::me_set(int me_type, bool is_signal) {
    // me_type: 0: MG5; 1: Physsim

    if (me_type == 0) {
        me_element = nullptr;
    } else if (me_type == 1) {
        if (is_signal) {
            me_element = new MEElementPhyssimZHH(&constants, -1., 1., 5); // 5,11 = mu-,b
        } else {
            me_element = new MEElementPhyssimZZH(&constants, -1., 1., 5, 11);
        }
    }
}
