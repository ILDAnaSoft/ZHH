...

{
 ...
 registerProcessorParameter("b1Mass",
				"mass of Boson b1 to be formed by pairing jets through a simple chi2",
				m_b1mass,
				float(125.)
				);

 registerProcessorParameter("b2Mass",
				"type of Boson b2 to be formed by pairing jets through a simple chi2",
				m_b2mass,
				float(125.)
				);
...
void Misclustering::processEvent( LCEvent* pLCEvent)
{
  ...
  // Collection with parameters which true jet belongs to which reconstructed jets
  // First two entries refer to first dijet, second two to second dijet
  ...
  try {
    ...
    streamlog_out(DEBUG3) << "Number of True Hadronic Jets(type = 1): " << m_nTrueJets << endl;
      ...
      m_pTTree->Fill();
      return;
    } else if ( m_nRecoJets != 4 ) {
      streamlog_out(DEBUG3) << "Number of jets is not equal to 4." << endl;
      m_pTTree->Fill();
      return;
    }
    ...
    if (foundTrueJets) {
      ...
      if(region.size() >= 2) {
        XX = string(region[0])+string(region[1]);
        m_regionXX.push_back(dict[XX]);
        m_jetMatchingCol->parameters().setValue("region", (int)dict[XX]);
      }

      if(region.size() >= 2) {
        XX_ICNs = string(region_ICNs[0])+string(region_ICNs[1]);
