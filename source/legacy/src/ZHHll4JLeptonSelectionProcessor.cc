...
void ZHHll4JLeptonSelectionProcessor::processEvent( LCEvent * evt ) { 
  ...
  for (Int_t i=0;i<nPFOs;i++) {
	...
	nHits = calHits.size();
      ...
      Double_t fCosLargeConeCut = 0.95;
      Int_t nConePhoton = 0;
	...
	if (nsigd0 < 50 && nsigz0 < 5) {   // contraint to primary vertex
	  if (_is_training_done) mva_electron = _readers[0]->EvaluateMVA( "MLP method"           );
	}
      }
      if (charge != 0 && 
	...
	if (nsigd0 < 5 && nsigz0 < 5) {
	  if (_is_training_done) mva_muon = _readers[1]->EvaluateMVA( "MLP method"           );
	}
      }
      ...
      data[46] = nsigr0;
      data[47] = iOverlay;
      data[48] = cosThetaWithLargeCone;
      data[49] = energyRatioWithLargeCone;
      data[50] = nConePhoton;
      data[51] = ratioECal;
      data[52] = ratioTotalCal;
      data[53] = mva_electron;
      data[54] = mva_muon;
      if (_is_lep_tune) {
	hPfo->Fill(data);
      }
