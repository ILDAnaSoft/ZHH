  ...
  eB(0.0)
{
  ...
  _description = "ttbarKinFit does a fit on 6 particle events with 4-momentum conservation and possiblity for mass constraints" ;
  ...
  */
  ...
  registerProcessorParameter("whichSignature",
			     "Which signature; lvbbqq", //bbqqqq to be added?
			     m_signature,
			     std::string("lvbbqq")
			     );
...
void ttbarKinFit::processEvent( EVENT::LCEvent *pLCEvent )
{
  ...
  */
  ...
  streamlog_out(MESSAGE) << "	||||||||||||||||||||||||||||||||||||||||||  KINFIT  ||||||||||||||||||||||||||||||||||||||||||" << std::endl ;
  streamlog_out(MESSAGE) << "	||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||" << std::endl ;
  
  FitResult FitResult;
  if (m_signature == "lvbbqq") {
    FitResult = performlvbbqqFIT( Jets, Leptons, traceEvent );
  }
  ...
  if (!FitResult.fitter) {
    ...
    streamlog_out(MESSAGE) << "error code = " << FitResult.fitter->getError() << endl;
    streamlog_out(MESSAGE) << "fit prob = " << FitResult.fitter->getProbability() << endl;
    streamlog_out(MESSAGE) << "fit chi2 = " << FitResult.fitter->getChi2()<< endl;
    ...
    auto fitobjects = FitResult.fitobjects;
...
ttbarKinFit::FitResult ttbarKinFit::performlvbbqqFIT( pfoVector jets, pfoVector leptons, bool traceEvent) {
...
ttbarKinFit::FitResult ttbarKinFit::performvvbbbbFIT( pfoVector jets, bool traceEvent) {
  shared_ptr<vector<shared_ptr<JetFitObject>>> jfo = make_shared<vector<shared_ptr<JetFitObject>>>();
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //////												  //////
  //////					Set JetFitObjects					  //////
...
ttbarKinFit::FitResult ttbarKinFit::performqqbbbbFIT( pfoVector jets, bool traceEvent) {
  shared_ptr<vector<shared_ptr<JetFitObject>>> jfo = make_shared<vector<shared_ptr<JetFitObject>>>();
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //////												  //////
  //////					Set JetFitObjects					  //////
...
std::vector<double> ttbarKinFit::calculateInitialMasses(pfoVector jets, pfoVector leptons, vector<int> perm)
{
...
std::vector<double> ttbarKinFit::calculateMassesFromSimpleChi2Pairing(pfoVector jets, pfoVector leptons) 
{
  std::vector<double> masses;
  vector<vector<unsigned int>> perms;
...
std::vector<double> ttbarKinFit::calculatePulls(std::shared_ptr<ParticleFitObject> fittedobject, ReconstructedParticle* startobject, int type)
{
...
double ttbarKinFit::calcChi2(shared_ptr<vector<shared_ptr<BaseFitObject>>> fitobjects) {
  double chi2 = 0;
  for (auto it = fitobjects->begin(); it != fitobjects->end(); ++it) {
...
void ttbarKinFit::check( LCEvent* )
{
