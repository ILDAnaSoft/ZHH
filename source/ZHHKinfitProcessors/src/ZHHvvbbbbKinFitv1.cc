...
ZHHvvbbbbKinFitv1 aZHHvvbbbbKinFitv1 ;

ZHHvvbbbbKinFitv1::ZHHvvbbbbKinFitv1() :
Processor("ZHHvvbbbbKinFitv1"),
m_nRun(0),
m_nEvt(0),
m_nRunSum(0),
m_nEvtSum(0),
...
//	modify processor description
	_description = "ZHHvvbbbbKinFitv1 does a kinematic fit on Z->invisible + 4 jet events" ;
...
//	  = 2*alpha/pi*( ln(s/m_e^2)-1 )
	...
	const char* treeName = (std::string("KinFitVV") + m_fithypothesis).c_str();
  ...
  m_pTTree->Branch("p1stBeforeFit_woNu" , &m_p1stBeforeFit_woNu , "p1stBeforeFit_woNu/F" );
  m_pTTree->Branch("cosThBeforeFit_woNu" , &m_cos1stBeforeFit_woNu , "cosThBeforeFit_woNu/F" );
  ...
  m_pTTree->Branch("cosThAfterFit_woNu" , &m_cos1stAfterFit_woNu , "cosThAfterFit_woNu/F" );
...
void ZHHvvbbbbKinFitv1::processRunHeader()
{
	m_nRun++ ;
}

void ZHHvvbbbbKinFitv1::processEvent( EVENT::LCEvent *pLCEvent )
{
  this->Clear();
...
ReconstructedParticle* ZHHvvbbbbKinFitv1::addNeutrinoCorrection(ReconstructedParticle* jet, pfoVector neutrinos) {
 ...
 */
std::vector<std::vector<EVENT::ReconstructedParticle*>> ZHHvvbbbbKinFitv1::combinations(pfoVectorVector collector,
...
std::vector<std::vector<EVENT::ReconstructedParticle*>> ZHHvvbbbbKinFitv1::getNeutrinosInJet( LCRelationNavigator* JetSLDNav , 
...
std::pair<MCParticle*,ReconstructedParticle*> ZHHvvbbbbKinFitv1::getMCNeutrino(LCRelationNavigator* NuMCNav,
										    LCRelationNavigator* SLDNuNav,
...
ZHHvvbbbbKinFitv1::FitResult ZHHvvbbbbKinFitv1::performFIT( pfoVector jets, 
							pfoVector leptons,
							bool traceEvent) {
  ...
  } else if (m_fithypothesis == "ZHH" || m_fithypothesis == "ZZZ" || m_fithypothesis == "EQM") {
    perms = {
      {0, 1, 2, 3}, 
      {0, 2, 1, 3}, 
      {0, 3, 1, 2}
...
void ZHHvvbbbbKinFitv1::getJetParameters(	ReconstructedParticle* jet, float (&parameters)[3], float (&errors)[3])
{
...
void ZHHvvbbbbKinFitv1::getLeptonParameters( ReconstructedParticle* lepton , float (&parameters)[ 3 ] , float (&errors)[ 3 ] )
{
...
std::vector<double> ZHHvvbbbbKinFitv1::calculateInitialMasses(pfoVector jets, pfoVector leptons, vector<unsigned int> perm, bool woNu)
{
...
std::vector<double> ZHHvvbbbbKinFitv1::calculateMassesFromSimpleChi2Pairing(pfoVector jets, pfoVector leptons) 
{
  std::vector<double> masses;
  vector<vector<unsigned int>> perms;
...
std::vector<double> ZHHvvbbbbKinFitv1::calculatePulls(std::shared_ptr<ParticleFitObject> fittedobject, ReconstructedParticle* startobject, int type)
{
...
double ZHHvvbbbbKinFitv1::calcChi2(shared_ptr<vector<shared_ptr<BaseFitObject>>> fitobjects) {
  double chi2 = 0;
  for (auto it = fitobjects->begin(); it != fitobjects->end(); ++it) {
...
void ZHHvvbbbbKinFitv1::check( LCEvent* )
{
//	nothing to check here - could be used to fill checkplots in reconstruction processor
}


void ZHHvvbbbbKinFitv1::end()
{
