...
ZHHKinFit::FitResult ZHHKinFit::performqqbbbbFIT( pfoVector jets, bool traceEvent, LCCollection *originalJetCollection) {
  ...
  if (m_fithypothesis == "ZZH" || m_fithypothesis == "ZZHsoft" || m_fithypothesis == "MH" || m_fithypothesis == "ZHH" || m_fithypothesis == "ZHHsoft" || m_fithypothesis == "EQM") {
    ...
    float T = 0.4 ; //cut threshold
    std::vector<unsigned int> quarks;
    std::vector<unsigned int> antiquarks;
    std::vector<unsigned int> ambiguous;

    //per event loop: every jet evaluate against cut, decision made whether they are ambiguous, q or q bar
    for (unsigned int i_jet = 0; i_jet < jets.size(); i_jet++) {
      ...
      int sign_tag = 0; //ambigious=0, q=1, qbar=-1

      if (logB > T || logC > T) sign_tag = 1;

      if (logB < -T || logC < -T) {
        if (sign_tag == 1) {
          sign_tag = 0; //ambigious case, conflicting sign assigment from logB & logC 
        } else {
          sign_tag = -1;
        }
      }
    
    //event overview to see which category they fall into
      if (sign_tag == 1) quarks.push_back(i_jet);
      else if (sign_tag == -1) antiquarks.push_back(i_jet);
      else ambiguous.push_back(i_jet);
    }

    //and hence their perms
    if (quarks.size() == 3 && antiquarks.size() == 3 && ambiguous.size() == 0) {
      vector<vector<unsigned int>> A_perms = { //hardcode all perms for qbar
        {antiquarks[0], antiquarks[1], antiquarks[2]}, {antiquarks[0], antiquarks[2], antiquarks[1]}, 
        {antiquarks[1], antiquarks[2], antiquarks[0]}, {antiquarks[1], antiquarks[0], antiquarks[2]}, 
        {antiquarks[2], antiquarks[0], antiquarks[1]}, {antiquarks[2], antiquarks[1], antiquarks[0]}
      };
      for (auto& Ap : A_perms) { //q pairing with each qbar perms and rotation to fit for Z/H
        perms.push_back({quarks[0], Ap[0], quarks[1], Ap[1], quarks[2], Ap[2]});
        perms.push_back({quarks[1], Ap[1], quarks[2], Ap[2], quarks[0], Ap[0]});
        perms.push_back({quarks[2], Ap[2], quarks[0], Ap[0], quarks[1], Ap[1]});
      }
    } else if (quarks.size() == 2 && antiquarks.size() == 4 && ambiguous.size() == 0) {
        for (int i = 0; i < 4; i++) { //to pair with 1st quark
          for (int j = 0; j < 4; j++) { //to pair with 2nd quark
            if (i == j) continue; //skip same anti q index

            vector<unsigned int> remainder;
            for (int k = 0; k < 4; k++) { 
              if (k != i && k != j) remainder.push_back(antiquarks[k]); } //2 unpaired with 1 false id push into new array "remainder"
            
            perms.push_back({quarks[0], antiquarks[i], quarks[1], antiquarks[j], remainder[0], remainder[1]});
            perms.push_back({remainder[0], remainder[1], quarks[0], antiquarks[i], quarks[1], antiquarks[j]});
            perms.push_back({quarks[1], antiquarks[j], remainder[0], remainder[1], quarks[0], antiquarks[i]});
          }
        }
      } else if (quarks.size() == 4 && antiquarks.size() == 2 && ambiguous.size() == 0) {
          for (int i = 0; i < 4; i++) { //to pair with 1st antiquark
            for (int j = 0; j < 4; j++) { //to pair with 2nd antiquark
              if (i == j) continue; //skip same q index

              vector<unsigned int> remainder;
              for (int k = 0; k < 4; k++) { 
                if (k != i && k != j) remainder.push_back(quarks[k]); } //2 unpaired with 1 false id push into new array "remainder"
              
              perms.push_back({quarks[i], antiquarks[0], quarks[j], antiquarks[1], remainder[0], remainder[1]});
              perms.push_back({remainder[0], remainder[1], quarks[i], antiquarks[0], quarks[j], antiquarks[1]});
              perms.push_back({quarks[j], antiquarks[1], remainder[0], remainder[1], quarks[i], antiquarks[0]});
            }
          } 
        } else { 
            perms = {
