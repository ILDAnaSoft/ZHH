...
ZHHKinFit::FitResult ZHHKinFit::performqqbbbbFIT( pfoVector jets, bool traceEvent, LCCollection *originalJetCollection) {
  ...
  // 1. Pair each jet's b-score with its original index
    std::vector<std::pair<float, unsigned int>> b_scores;
    for (unsigned int i = 0; i < jets.size(); i++) {
        b_scores.push_back({std::max((float)m_scoreB[i], 1e-10f), i});
    }

    // 2. Sort the jets in descending order (highest b-score first)
    std::sort(b_scores.begin(), b_scores.end(), [](const auto& a, const auto& b) {
        return a.first > b.first;
    });

    unsigned int b1 = b_scores[0].second; // bmax1 
    unsigned int b2 = b_scores[1].second;
    unsigned int b3 = b_scores[2].second;
    unsigned int b4 = b_scores[3].second;
    unsigned int b5 = b_scores[4].second;
    unsigned int b6 = b_scores[5].second; // bmax6 

    // Calculate the total sum of all 6 b-tags
    float btag_sum = b_scores[0].first + b_scores[1].first + b_scores[2].first + 
                     b_scores[3].first + b_scores[4].first + b_scores[5].first;

    // Apply the logic threshold
    if (btag_sum < 4.0) { 
        
        if (m_fithypothesis == "ZHH" || m_fithypothesis == "ZHHsoft" || m_fithypothesis == "EQM") {
            // Assuming array order: Z(0,1), H1(2,3), H2(4,5)
            perms = {
                {b5, b6,  b1, b2,  b3, b4}, 
                {b5, b6,  b1, b3,  b2, b4}, 
                {b5, b6,  b1, b4,  b2, b3}  
        ...
        else if (m_fithypothesis == "ZZH" || m_fithypothesis == "ZZHsoft" || m_fithypothesis == "MH") {
            // LOGIC: 
            // - bmax5,6 not higgs (Locked to the first Z boson)
            // - bmax1-4 -> Z & H (Distributed between the second Z and the Higgs)
            // - 6 perms
            
            // Assuming array order: Z1(0,1), Z2(2,3), H(4,5)
            perms = {
                // If b1, b2 go to Higgs; b3, b4 go to Z2
                {b5, b6,  b3, b4,  b1, b2}, 
                // If b3, b4 go to Higgs; b1, b2 go to Z2
                {b5, b6,  b1, b2,  b3, b4}, 

                // If b1, b3 go to Higgs; b2, b4 go to Z2
                {b5, b6,  b2, b4,  b1, b3},
                // If b2, b4 go to Higgs; b1, b3 go to Z2
                {b5, b6,  b1, b3,  b2, b4},

                // If b1, b4 go to Higgs; b2, b3 go to Z2
                {b5, b6,  b2, b3,  b1, b4},
                // If b2, b3 go to Higgs; b1, b4 go to Z2
                {b5, b6,  b1, b4,  b2, b3}
            };
        }
    }
  ...
  if (m_fithypothesis == "ZZH" || m_fithypothesis == "ZZHsoft" || m_fithypothesis == "MH" || m_fithypothesis == "ZHH" || m_fithypothesis == "ZHHsoft" || m_fithypothesis == "EQM") {
    ...
    for (unsigned int i_jet = 0; i_jet < jets.size(); i_jet++) {
      ...
      int sign_tag = 0; 
      if (logB > ratiocut || logC > ratiocut) sign_tag = 1;
      if (logB < -ratiocut || logC < -ratiocut) {
          if (sign_tag == 1) sign_tag = 0; 
          else sign_tag = -1;
      }
    ...
    //assign the 2 tags to 'a, b' and the 4 remaining jets to 'w, x, y, z'
      ...
      unsigned int a, b;              //2 jets to separate
      vector<unsigned int> rem;       //4 remaining jets

      if (quarks.size() == 2) {
          a = quarks[0]; 
          b = quarks[1];
          //the remainder are just the other lists merged together
          for (int j : antiquarks) rem.push_back(j);
          for (int j : ambiguous)  rem.push_back(j);
      } 
      else if (antiquarks.size() == 2) {
          a = antiquarks[0]; 
          b = antiquarks[1];
          for (int j : quarks)     rem.push_back(j);
          for (int j : ambiguous)  rem.push_back(j);
      } 
      else if (quarks.size() == 4) {
          //if 4 quarks, separate the 2 NON-quarks
          vector<unsigned int> non_quarks;
          for (int j : antiquarks) non_quarks.push_back(j);
          for (int j : ambiguous)  non_quarks.push_back(j);
          
          a = non_quarks[0]; 
          b = non_quarks[1];
          rem = quarks; //remainder are the 4 quarks
      } 
      else if (antiquarks.size() == 4) {
          //if 4 antiquarks, separate the 2 NON-antiquarks
          vector<unsigned int> non_anti;
          for (int j : quarks)     non_anti.push_back(j);
          for (int j : ambiguous)  non_anti.push_back(j);
          
          a = non_anti[0]; 
          b = non_anti[1];
          rem = antiquarks; //remainder are the 4 antiquarks
      }
