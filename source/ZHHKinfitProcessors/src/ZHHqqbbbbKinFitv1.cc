...
ZHHqqbbbbKinFitv1 aZHHqqbbbbKinFitv1 ;

ZHHqqbbbbKinFitv1::ZHHqqbbbbKinFitv1() :
Processor("ZHHqqbbbbKinFitv1"),
m_nRun(0),
m_nEvt(0),
m_nRunSum(0),
m_nEvtSum(0),
...
//	modify processor description
	_description = "ZHHqqbbbbKinFitv1 does a kinematic fit on 6 jet events" ;
...
//	  = 2*alpha/pi*( ln(s/m_e^2)-1 )
	...
	const char* treeName = (std::string("KinFitQQ") + m_fithypothesis).c_str();
...
void ZHHqqbbbbKinFitv1::processRunHeader()
{
	m_nRun++ ;
}

void ZHHqqbbbbKinFitv1::processEvent( EVENT::LCEvent *pLCEvent )
{
  this->Clear();
...
ReconstructedParticle* ZHHqqbbbbKinFitv1::addNeutrinoCorrection(ReconstructedParticle* jet, pfoVector neutrinos) {
 ...
 */
std::vector<std::vector<EVENT::ReconstructedParticle*>> ZHHqqbbbbKinFitv1::combinations(pfoVectorVector collector,
...
std::vector<std::vector<EVENT::ReconstructedParticle*>> ZHHqqbbbbKinFitv1::getNeutrinosInJet( LCRelationNavigator* JetSLDNav , 
...
std::pair<MCParticle*,ReconstructedParticle*> ZHHqqbbbbKinFitv1::getMCNeutrino(LCRelationNavigator* NuMCNav,
										    LCRelationNavigator* SLDNuNav,
...
ZHHqqbbbbKinFitv1::FitResult ZHHqqbbbbKinFitv1::performFIT( pfoVector jets, 
							pfoVector leptons,
							bool traceEvent) {
...
void ZHHqqbbbbKinFitv1::getJetParameters(	ReconstructedParticle* jet, float (&parameters)[3], float (&errors)[3])
{
...
void ZHHqqbbbbKinFitv1::getLeptonParameters( ReconstructedParticle* lepton , float (&parameters)[ 3 ] , float (&errors)[ 3 ] )
{
...
std::vector<double> ZHHqqbbbbKinFitv1::calculateInitialMasses(pfoVector jets, pfoVector leptons, vector<unsigned int> perm, bool woNu)
{
...
std::vector<double> ZHHqqbbbbKinFitv1::calculateMassesFromSimpleChi2Pairing(pfoVector jets, pfoVector leptons) 
{
  std::vector<double> masses;
  vector<vector<unsigned int>> perms;
...
std::vector<double> ZHHqqbbbbKinFitv1::calculatePulls(std::shared_ptr<ParticleFitObject> fittedobject, ReconstructedParticle* startobject, int type)
{
...
double ZHHqqbbbbKinFitv1::calcChi2(shared_ptr<vector<shared_ptr<BaseFitObject>>> fitobjects) {
  double chi2 = 0;
  for (auto it = fitobjects->begin(); it != fitobjects->end(); ++it) {
...
void ZHHqqbbbbKinFitv1::check( LCEvent* )
{
//	nothing to check here - could be used to fill checkplots in reconstruction processor
}


void ZHHqqbbbbKinFitv1::end()
{
