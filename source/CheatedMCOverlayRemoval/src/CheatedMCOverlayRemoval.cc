  ...
  m_nMCPs(0)
{
	...
	_description = "CheatedMCOverlayRemoval identifies MC particles that are overlay and removes their PFOs from the collection" ;
...
void CheatedMCOverlayRemoval::processEvent( LCEvent *pLCEvent )
{
  ...
  try
	...
	if (mcp->isOverlay()) { // && mcp->getGeneratorStatus() == 1) { 
	...
	if ( linkedMCP != NULL ) {
	  //streamlog_out(DEBUG5) << "i = " << i << ": Linked MCP gen status = " << linkedMCP->getGeneratorStatus() << std::endl;
	  //if (linkedMCP->isOverlay()) streamlog_out(DEBUG5) << "Linked MCP is from overlay" << std::endl;
	  if (linkedMCP->isOverlay()) streamlog_out(DEBUG5) << "i = " << i << ": energy = " << pfo->getEnergy () << ", gen status = " << linkedMCP->getGeneratorStatus() << std::endl;
	  //if (linkedMCP->isOverlay() && linkedMCP->getGeneratorStatus() == 1) continue;
	...
	nKeptPFOs++;
      ...
      //if (nKeptPFOs + nRemovedPFOs != m_nAllPFOs) 
      streamlog_out(MESSAGE) << "In event " << m_nEvt << ": Kept PFOs = " << nKeptPFOs << " VS removed number of PFOs = " << nRemovedPFOs << " VS total number of PFOs = " << m_nAllPFOs << std::endl;
      pLCEvent->addCollection(OutputPfoCollection, _OutputPfoCollection.c_str() );
      //pLCEvent->addCollection(OutputPfoCollection, _OutputPfoCollection.c_str() );
      //pLCEvent->addCollection(OutputHiggs1Collection, _OutputHiggs1Collection.c_str() );
      //pLCEvent->addCollection(OutputHiggs2Collection, _OutputHiggs2Collection.c_str() );

      //Loop over PandoraPFOs to see which particles are removed
      /*if (nKeptPFOs != m_nAllPFOs) {
	for (int i=0; i<m_nAllPFOs; i++) {
	  ReconstructedParticle* pfo = (ReconstructedParticle*) AllPFOs->getElementAt(i);
	  if (std::find(OutputPfoCollection->begin(), OutputPfoCollection->end(), pfo) == OutputPfoCollection->end()) 
	    streamlog_out(MESSAGE) <<  "id  = " << pfo->id() << ", type = " << pfo->getType() << ", momentum = " << pfo->getMomentum()[0] << ", " << pfo->getMomentum()[1] << ", " << pfo->getMomentum()[2] << ", energy = " << pfo->getEnergy () << std::endl;
	    }
    }*/
